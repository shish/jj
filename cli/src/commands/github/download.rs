...
pub async fn cmd_github_download(
    ui: &mut Ui,
    ...
    let branch_name = get_pr_head_ref_name(&client, pr_number)?;
    ...
    tx.finish(
        ui,
        format!("create working copy for GitHub pull request #{pr_number}"),
    )
    .await?;
...
fn get_pr_head_ref_name(client: &GitHubClient, pr_number: i64) -> Result<String, CommandError> {
    ...
    // TODO: add {headRepositoryOwner.login}/{headRepository.name}
    // as a remote and pull from there? Pull from the secret
    // `{targetRepo}/pull/{pr_number}` ref?
    if data["repository"]["pullRequest"]["isCrossRepository"]
        .as_bool()
        .unwrap_or(false)
    {
    ...
    data["repository"]["pullRequest"]["headRefName"]
        ...
        .ok_or_else(|| user_error(format!("PR #{pr_number} is missing headRefName")))
}
